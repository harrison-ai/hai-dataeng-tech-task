...
similar in shape to the sample data from question 1, but we have several petabytes
of archives, where each is around 1 GB.
...
Provide your solution by commiting a design document (as well as any supporting materials) for a
data-processing pipeline that can re-assemble these DICOMs at scale and make the resulting
`.dcm` files available as objects in an S3 bucket. The system will need to run in Amazon AWS,
and you're encouraged to be specific about technology choices.
